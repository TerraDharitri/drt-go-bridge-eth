...
import (
	...
	cryptoMocks "github.com/TerraDharitri/drt-go-bridge-eth/testsCommon/crypto"
	p2pMocks "github.com/TerraDharitri/drt-go-bridge-eth/testsCommon/p2p"
	roleProvidersMock "github.com/TerraDharitri/drt-go-bridge-eth/testsCommon/roleProviders"
...
func createMockArgsBroadcaster() ArgsBroadcaster {
	...
	return ArgsBroadcaster{
		...
		DharitrIRoleProvider: &roleProvidersMock.DharitrIRoleProviderStub{},
...
func TestNewBroadcaster(t *testing.T) {
	...
	t.Run("nil DharitrI role provider should error", func(t *testing.T) {
		args := createMockArgsBroadcaster()
		args.DharitrIRoleProvider = nil

		b, err := NewBroadcaster(args)
		assert.True(t, check.IfNil(b))
		assert.Equal(t, ErrNilDharitrIRoleProvider, err)
	})
...
func TestBroadcaster_ProcessReceivedMessage(t *testing.T) {
	...
	t.Run("pre process fails", func(t *testing.T) {
		...
		_, err := b.ProcessReceivedMessage(p2pMsg, "", nil)
		assert.NotNil(t, err)
	})
	t.Run("public key not whitelisted", func(t *testing.T) {
		...
		args.DharitrIRoleProvider = &roleProvidersMock.DharitrIRoleProviderStub{
	...
	t.Run("joined topic should send stored messages from clients", func(t *testing.T) {
		...
		_, _ = b.ProcessReceivedMessage(p2pMsg, "", nil)
	...
	t.Run("system busy on a topic", func(t *testing.T) {
		...
		_, err = b.ProcessReceivedMessage(p2pMsg, "p1", nil)
		assert.Nil(t, err)
