...
// GetBatchSCMetadata returns the emitted logs in a batch that hold metadata for SC execution on DRT
...
// WasExecuted returns true if the DharitrI batch ID was executed
func (c *client) WasExecuted(ctx context.Context, drtBatchID uint64) (bool, error) {
	return c.clientWrapper.WasBatchExecuted(ctx, big.NewInt(0).SetUint64(drtBatchID))
}
...
func (c *client) setStatusForAvailabilityCheck(status core.ClientStatus, message string, nonce uint64) {
	c.clientWrapper.SetStringMetric(core.MetricDharitrIClientStatus, status.String())
	c.clientWrapper.SetStringMetric(core.MetricLastDharitrIClientError, message)
