...
func TestClient_ExecuteTransfer(t *testing.T) {
	...
	t.Run("not enough balance for fees", func(t *testing.T) {
		...
		newArgLists := batchProcessor.ExtractListFromDrt(newBatch)
...
func resetClient(c *client) {
	...
	c.mut.Unlock()
	c.clientWrapper.SetStringMetric(bridgeCore.MetricDharitrIClientStatus, "")
	c.clientWrapper.SetStringMetric(bridgeCore.MetricLastDharitrIClientError, "")
}
