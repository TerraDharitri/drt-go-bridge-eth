...
const (
	proposeTransferFuncName         = "proposeMultiTransferDcdtBatch"
	proposeSetStatusFuncName        = "proposeDcdtSafeSetCurrentTransactionBatchStatus"
	signFuncName                    = "sign"
	performActionFuncName           = "performAction"
	minClientAvailabilityAllowDelta = 1
	prefixBytesLength               = 4

	dharitrIDataGetterLogId = "DharitrIEth-DharitrIDataGetter"
)

// ClientArgs represents the argument for the NewClient constructor function
type ClientArgs struct {
	GasMapConfig                 config.DharitrIGasMapConfig
	Proxy                        Proxy
	Log                          logger.Logger
...
// client represents the DharitrI Client implementation
type client struct {
	*drtClientDataGetter
	txHandler                    txHandler
	tokensMapper                 TokensMapper
...
// NewClient returns a new DharitrI Client instance
func NewClient(args ClientArgs) (*client, error) {
	...
	argsDRTClientDataGetter := ArgsDRTClientDataGetter{
		MultisigContractAddress: args.MultisigContractAddress,
		SafeContractAddress:     args.SafeContractAddress,
		RelayerAddress:          relayerAddress,
		Proxy:                   args.Proxy,
		Log:                     bridgeCore.NewLoggerWithIdentifier(logger.GetOrCreate(dharitrIDataGetterLogId), dharitrIDataGetterLogId),
	}
	getter, err := NewDRTClientDataGetter(argsDRTClientDataGetter)
	if err != nil {
		return nil, err
	}
...
func checkGasMapValues(gasMap config.DharitrIGasMapConfig) error {
...
// ProposeSetStatus will trigger the proposal of the DCDT safe set current transaction batch status operation
...
func (c *client) CheckRequiredBalance(ctx context.Context, token []byte, value *big.Int) error {
	...
	dcdt, err := c.proxy.GetDCDTTokenData(ctx, c.safeContractAddress, string(token), api.AccountQueryOptions{})
	if err != nil {
		return fmt.Errorf("%w for address %s for DCDT token %s", err, safeAddress, string(token))
	}

	existingBalance, ok := big.NewInt(0).SetString(dcdt.Balance, 10)
	if !ok {
		return fmt.Errorf("%w for DCDT token %s and address %s", errInvalidBalance, string(token), safeAddress)
	}

	if value.Cmp(existingBalance) > 0 {
		return fmt.Errorf("%w, existing: %s, required: %s for ERC20 token %s and address %s",
			errInsufficientDCDTBalance, existingBalance.String(), value.String(), string(token), safeAddress)
	}

	c.log.Debug("checked ERC20 balance",
		"DCDT token", string(token),
		"address", safeAddress,
