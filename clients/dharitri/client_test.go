...
func createMockClientArgs() ClientArgs {
	...
	multisigContractAddress, _ := data.NewAddressFromBech32String("drt1qqqqqqqqqqqqqpgqzyuaqg3dl7rqlkudrsnm5ek0j3a97qevd8sslwctuh")
	safeContractAddress, _ := data.NewAddressFromBech32String("drt1qqqqqqqqqqqqqpgqtvnswnzxxz8susupesys0hvg7q2z5nawrcjqjxhwlw")
			...
			Deposits: []*bridgeCore.DepositTransfer{
				{
					...
					DisplayableFrom:       "drt1qyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszxwvzq",
				...
				},
				{
					...
					DisplayableFrom:       "drt1qszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqmw709d",
...
func TestClient_CheckClientAvailability(t *testing.T) {
	...
	t.Run("same current nonce should error after a while", func(t *testing.T) {
		...
		statusHandler.SetStringMetric(bridgeCore.MetricDharitrIClientStatus, bridgeCore.ClientStatus(3).String())
		statusHandler.SetStringMetric(bridgeCore.MetricLastDharitrIClientError, "random")
...
func resetClient(c *client) {
	...
	c.mut.Unlock()
	c.statusHandler.SetStringMetric(bridgeCore.MetricDharitrIClientStatus, "")
	c.statusHandler.SetStringMetric(bridgeCore.MetricLastDharitrIClientError, "")
...
func checkStatusHandler(t *testing.T, statusHandler *testsCommon.StatusHandlerMock, status bridgeCore.ClientStatus, message string) {
	assert.Equal(t, status.String(), statusHandler.GetStringMetric(bridgeCore.MetricDharitrIClientStatus))
	assert.Equal(t, message, statusHandler.GetStringMetric(bridgeCore.MetricLastDharitrIClientError))
}
