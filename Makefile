...
slow-tests: clean-test
	...
	@bash -c 'for i in $(seq 1 60); do if curl -s http://localhost:9000/ >/dev/null 2>&1 && curl -s http://localhost:9123/ >/dev/null 2>&1; then exit 0; fi; sleep 1; done; echo "Sui network did not become ready in time" >&2; exit 1'
