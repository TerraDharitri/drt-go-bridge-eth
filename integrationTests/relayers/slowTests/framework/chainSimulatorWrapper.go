...
const (
	...
	thousandRewa                            = "1000000000000000000000"
...
func (instance *chainSimulatorWrapper) Proxy() dharitri.Proxy {
...
func (instance *chainSimulatorWrapper) DeploySC(ctx context.Context, wasmFilePath string, ownerSK []byte, gasLimit uint64, parameters []string) (*DrtAddress, string, *data.TransactionOnNetwork) {
	...
	return NewDrtAddressFromBech32(instance.TB, txResult.Logs.Events[0].Address), hash, txResult
}
...
func (instance *chainSimulatorWrapper) ScCall(ctx context.Context, senderSK []byte, contract *DrtAddress, value string, gasLimit uint64, function string, parameters []string) (string, *data.TransactionOnNetwork) {
...
func (instance *chainSimulatorWrapper) ScCallWithoutGenerateBlocks(ctx context.Context, senderSK []byte, contract *DrtAddress, value string, gasLimit uint64, function string, parameters []string) string {
...
func (instance *chainSimulatorWrapper) SendTx(ctx context.Context, senderSK []byte, receiver *DrtAddress, value string, gasLimit uint64, dataField []byte) (string, *data.TransactionOnNetwork) {
...
func (instance *chainSimulatorWrapper) SendTxWithoutGenerateBlocks(ctx context.Context, senderSK []byte, receiver *DrtAddress, value string, gasLimit uint64, dataField []byte) string {
	networkConfig, err := instance.proxyInstance.GetNetworkConfig(ctx)
	require.Nil(instance, err)
...
// GetDCDTBalance returns the balance of the dcdt token for the provided address
func (instance *chainSimulatorWrapper) GetDCDTBalance(ctx context.Context, address *DrtAddress, token string) string {
	tokenData, err := instance.proxyInstance.GetDCDTTokenData(ctx, address, token, apiCore.AccountQueryOptions{
		OnFinalBlock: true,
	})
	require.Nil(instance, err)
...
func (instance *chainSimulatorWrapper) GetBlockchainTimeStamp(ctx context.Context) uint64 {
	...
	resultStruct := struct {
		Data struct {
			Status struct {
				DrtBlockTimestamp uint64 `json:"drt_block_timestamp"`
			} `json:"status"`
		} `json:"data"`
	}{}
	...
	return resultStruct.Data.Status.DrtBlockTimestamp
}
