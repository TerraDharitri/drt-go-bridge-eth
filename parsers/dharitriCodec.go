...
const lenDrtAddress = 32

// DharitriCodec defines the codec operations to be used for DharitrI contracts
type DharitriCodec struct {
}
...
func (codec *DharitriCodec) DecodeProxySCCompleteCallData(buff []byte) (ProxySCCompleteCallData, error) {
	...
	if len(buff) < lenDrtAddress {
		return ProxySCCompleteCallData{}, errBufferTooShortForDrtAddress
	}
	result.To = data.NewAddressFromBytes(buff[:lenDrtAddress])
	buff = buff[lenDrtAddress:]
...
func (codec *DharitriCodec) ExtractGasLimitFromRawCallData(buff []byte) (uint64, error) {
	if len(buff) == 0 {
...
func (codec *DharitriCodec) IsInterfaceNil() bool {
	return codec == nil
}
