...
func createTestProxySCCompleteCallData() ProxySCCompleteCallData {
	...
	completeCallData.To, _ = data.NewAddressFromBech32String("drt1qqqqqqqqqqqqqpgqsudu3a3n9yu62k5qkgcpy4j9ywl2x2gl5smsezuklj")
	completeCallData.From.SetBytes(ethUnhexed)

	return completeCallData
}

func TestDharitriCodec_IsInterfaceNil(t *testing.T) {
	t.Parallel()

	var instance *DharitriCodec
	assert.True(t, instance.IsInterfaceNil())

	instance = &DharitriCodec{}
	assert.False(t, instance.IsInterfaceNil())
}

func TestDharitriCodec_ExtractGasLimitFromRawCallData(t *testing.T) {
	t.Parallel()
...
func TestDharitriCodec_DecodeProxySCCompleteCallData(t *testing.T) {
	t.Parallel()
	...
	t.Run("buffer to short for DharitrI address should error", func(t *testing.T) {
		t.Parallel()

		buff := bytes.Repeat([]byte{0x01}, 20)
		buff = append(buff, 0x1)
		completeCallData, err := codec.DecodeProxySCCompleteCallData(buff)
		assert.ErrorIs(t, err, errBufferTooShortForDrtAddress)
		assert.Equal(t, emptyCompleteCallData, completeCallData)
	})
	t.Run("invalid token size bytes should error", func(t *testing.T) {
		...
		buff = append(buff, bytes.Repeat([]byte{0x01}, 32)...) // Drt address
