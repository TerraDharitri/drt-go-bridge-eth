...
// TestDharitrICodec is the codec helper used in testing
type TestDharitrICodec struct {
}
...
func (codec *TestDharitrICodec) EncodeCallDataWithLenAndMarker(callData parsers.CallData) []byte {
...
func (codec *TestDharitrICodec) EncodeCallDataStrict(callData parsers.CallData) []byte {
	initialAlloc := 1024 * 1024 // 1MB initial buffer
	result := make([]byte, 0, initialAlloc)
...
func (codec *TestDharitrICodec) encodeArgs(args []string) []byte {
	buff32Bits := make([]byte, 4)
...
func (codec *TestDharitrICodec) DecodeCallData(buff []byte) parsers.CallData {
	if len(buff) == 0 {
		panic("empty buffer")
	}
